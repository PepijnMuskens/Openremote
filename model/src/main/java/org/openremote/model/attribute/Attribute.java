...
public class Attribute<T> extends AbstractNameValueHolder<T> implements MetaHolder {
    ...
    private T CheckConstraints(T value){
        this.hasMeta("constraints");
            if (this.meta != null) {
                if (this.meta.getValue("constraints").isPresent()) {
                    var constraintsArr = (ValueConstraint[])(this.meta.getValue("constraints").stream().toArray()[0]);
                    for (var con : constraintsArr) {
                        if (con.getClass() == ValueConstraint.Size.class) {
                            if (value.getClass() == Double.class || value.getClass() == Integer.class || value.getClass() == Long.class ||
                                value.getClass() == BigInteger.class || value.getClass() == BigDecimal.class ) {
                                double val = ((Number) value).doubleValue();
                                var c = (ValueConstraint.Size) con;
                                if (c.getMax().isPresent()) {
                                    if (c.getMax().get() < val) {
                                        value = this.value;
                                    }
                                }
                                if (c.getMin().isPresent()) {
                                    if (c.getMin().get() > val) {
                                        value = this.value;
                                    }
                                }
                        ...
                        if (con.getClass() == ValueConstraint.Min.class) {
                            if (value.getClass() == Double.class || value.getClass() == Integer.class || value.getClass() == Long.class ||
                                value.getClass() == BigInteger.class || value.getClass() == BigDecimal.class ) {
                                double val = ((Number) value).doubleValue();
                                var c = (ValueConstraint.Min) con;
                                if (c.getMin().doubleValue() > val) {
                                    value = this.value;
                                }
                            }
                        }

                        if (con.getClass() == ValueConstraint.Max.class) {
                            if (value.getClass() == Double.class || value.getClass() == Integer.class || value.getClass() == Long.class ||
                                value.getClass() == BigInteger.class || value.getClass() == BigDecimal.class ) {
                                double val = ((Number) value).doubleValue();
                                var c = (ValueConstraint.Max) con;
                                if (c.getMax().doubleValue() < val) {
                                    value = this.value;
                                }
                            }
                        }
                        //todo add constraints enforcement for all types
                        if (con.getClass() == ValueConstraint.NotBlank.class) {
