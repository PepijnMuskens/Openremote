...
export class OrAttributeHistory extends translate(i18next)(LitElement) {
    ...
    protected async _loadData() {
        ...
        if (response.status === 200) {
            let data = response.data.filter(value => value.y !== null && value.y !== undefined) as ScatterDataPoint[];
            let anomalies =  anomalyresponse.data;

            anomalies.forEach(anomaly =>{
                let index = 1;
                if(index !== -1){
                    data[index].y = data[index].y +10;
                }
            })
